-- Update get_meta_events_journey to include the 6 new events
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'quiz_q5_budget_qualified', COUNT(DISTINCT CASE 
      ...
      THEN session_id 
    END),
    'quiz_q6_urgency', COUNT(DISTINCT CASE 
      WHEN event_name = 'ViewContent' AND content_category = 'quiz_q6_urgency' 
      THEN session_id 
    END),
    'quiz_q7_decision_maker', COUNT(DISTINCT CASE 
      WHEN event_name = 'ViewContent' AND content_category = 'quiz_q7_decision_maker' 
      THEN session_id 
    END),
