-- Filtrar UTMs por actividad reciente (últimos 3 días) para reducir ruido histórico
  ...
  utm_performance AS (
    ...
    FROM (
      ...
      WHERE (quiz_version_filter = 'all' OR qa.quiz_version = quiz_version_filter)
        -- NUEVO FILTRO: Solo mostrar UTMs con actividad en los últimos 3 días
        AND EXISTS (
          SELECT 1 FROM quiz_analytics qa2
          WHERE COALESCE(qa2.utm_source, 'organic') = COALESCE(qa.utm_source, 'organic')
            AND COALESCE(qa2.utm_medium, 'none') = COALESCE(qa.utm_medium, 'none')
            AND COALESCE(qa2.utm_campaign, 'none') = COALESCE(qa.utm_campaign, 'none')
            AND qa2.created_at >= NOW() - INTERVAL '3 days'
        )
      GROUP BY 
