...
import { PhoneCall, Handshake, type LucideIcon } from "lucide-react";

/** Bloque de manual (markdown) estilado on-brand (carbón/glow), sin @tailwindcss/typography. */
const ScriptCard = ({
  icon: Icon,
  eyebrow,
  title,
    ...
    <EnergyCardHeader>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-foreground/50 mt-0.5">{eyebrow}</p>
        </div>
      </div>
    </EnergyCardHeader>
    <EnergyCardContent>
      {copy && copy.trim() ? (
        ...
        <div className="flex flex-col items-center justify-center text-center gap-3 py-12 rounded-xl border border-white/[0.07] bg-white/[0.02]">
          ...
          <p className="text-sm text-foreground/55 max-w-xs">{emptyHint}</p>
        </div>
      )}
    </EnergyCardContent>
  </EnergyCard>
...
/** Sección "Guiones" del portal: entregas al cliente el manual de setting (agendar/cualificar)
 * y el manual de closing (llamada de venta) en markdown, on-brand. */
export const GuionesSection = ({
  settingCopy,
  closingCopy,
}: {
  settingCopy?: string | null;
  closingCopy?: string | null;
}) => (
  <div className="space-y-4">
    <ScriptCard
      icon={PhoneCall}
      eyebrow="Agendar, cualificar y asegurar el show"
      title={<>Manual de <span className="glow">Setting</span></>}
      copy={settingCopy}
      emptyHint="Tu manual de setting aparecerá aquí cuando esté listo. Lo escribimos durante el proyecto."
    />
    <ScriptCard
      icon={Handshake}
      eyebrow="La llamada de venta y el manejo de objeciones"
      title={<>Manual de <span className="glow">Closing</span></>}
      copy={closingCopy}
      emptyHint="Tu manual de closing aparecerá aquí cuando esté listo. Lo escribimos durante el proyecto."
    />
  </div>
);
