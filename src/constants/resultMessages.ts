export const PAIN_HEADLINES: Record<string, string> = {
  "Mis clientes vienen por recomendación de otros que pagaron poco (y son iguales o peores)": "Deja de heredar clientes rácanos",
  "Trabajamos muchas horas y el margen no justifica el esfuerzo del equipo": "Más margen, menos horas",
  "Tenemos meses buenos pero luego nos estampamos (dependemos de la suerte)": "Se acabó depender de la suerte",
  "No sé cómo vender proyectos de 5 cifras sin que nos regateen": "Proyectos de 5 cifras sin regateos",
  "Todo lo anterior (¿Pero de verdad se puede escalar esto?)": "Todo cambia en 30 días",
};

export const RESULT_MESSAGES = {
  qualified: {
    title: "Tu plaza está lista",
    subtitle: "Hablemos",
    socialProof: "FLOC facturó €80K en 4 días. Proyectos de hasta €60K cerrados dentro.",
    formCta: "APLICAR AL CÍRCULO →",
    postSubmit: "Elige tu hueco. La Senda te llega por WhatsApp para que vengas a la llamada con la cabeza lista.",
    trustSignal: "🔒 Pon un WhatsApp real. Lo usamos para confirmar tu cita y mandarte el material previo a la llamada.",
  },
  notQualified: {
    title: "La Senda no es para ti",
    description: "Esto no es un curso. No es para curiosos. No es para quienes no pueden invertir, no tienen prisa o necesitan que otro les dé permiso para decidir.",
    footer: "Cuando dejes de explorar y estés listo para ejecutar, quizá nos crucemos. O quizá no."
  }
};
